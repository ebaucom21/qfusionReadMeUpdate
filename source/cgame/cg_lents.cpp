...
void CG_RocketExplosionMode( const vec3_t pos, const vec3_t dir, int fire_mode, float radius ) {
	...
	// Disallow the "dust" for the strong cluster explosion mode
	if( fire_mode != FIRE_MODE_STRONG && cg_explosionsDust->integer ) {
	...
	if( fire_mode == FIRE_MODE_STRONG ) {
		...
		SoundSystem::Instance()->StartFixedSound( cgs.media.sfxRocketLauncherWeakHit, pos, CHAN_AUTO, cg_volume_effects->value, ATTN_DISTANT );
	}

	if( cg_heavyRocketExplosions->integer ) {
		SoundSystem::Instance()->StartFixedSound( cgs.media.sfxExplosionLfe, pos, CHAN_AUTO, cg_volume_effects->value, ATTN_NORM );
	}
...
void CG_WaveExplosionMode( const vec3_t pos, const vec3_t dir, int fire_mode, float radius ) {
	...
	le = CG_AllocModel( LE_INVERSESCALE_ALPHA_FADE, origin, vec3_origin, 3,
						...
						cgs.media.modWaveExplosion, NULL );
	...
	le = CG_AllocSprite( LE_SCALE_ALPHA_FADE, origin, radius * 0.3f, 3,
						 1, 1, 1, 1,
						 0, 0, 0, 0, // no dlight
						 cgs.media.shaderWaveExplosionRing );
	...
	if( fire_mode == FIRE_MODE_STRONG ) {
		...
		SoundSystem::Instance()->StartFixedSound( cgs.media.sfxWaveWeakHit, pos, CHAN_AUTO, cg_volume_effects->value, ATTN_DISTANT );
	}
...
#if 0
	struct shader_s *shader = cgs.media2.shaderRocketTrailSmokePuff;
#else
	struct shader_s *shader = cgs.media.shaderSmokePuff;
#endif
...
void CG_CartoonHitEffect( const vec3_t origin, const vec3_t dir, int damage ) {
	...
	struct shader_s *shader = cgs.media.shaderCartoonHit;
	struct shader_s *shader2 = cgs.media.shaderCartoonHit2;
	struct shader_s *shader3 = cgs.media.shaderCartoonHit3;
	vec3_t local_origin, local_dir;
...
void CG_GrenadeExplosionMode( const vec3_t pos, const vec3_t dir, int fire_mode, float radius ) {
	...
	if( fire_mode == FIRE_MODE_STRONG ) {
		...
		SoundSystem::Instance()->StartFixedSound( cgs.media.sfxGrenadeWeakExplosion, pos, CHAN_AUTO, cg_volume_effects->value, ATTN_DISTANT );
	}
